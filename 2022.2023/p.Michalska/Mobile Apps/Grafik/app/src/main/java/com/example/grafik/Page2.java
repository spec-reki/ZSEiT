...
public class Page2 extends AppCompatActivity implements View.OnClickListener, CalendarView.OnDateChangeListener  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (dates.isEmpty()) {
            dates.add("Nic nie planujesz na dziś");
        }
    ...
    public void onSelectedDayChange(@NonNull CalendarView view, int year, int month, int dayOfMonth) {
        ...
        if (selectedDate.get(Calendar.DAY_OF_WEEK) == Calendar.FRIDAY) {
            dates.add("BIWACZEK");
            dates.remove("NIE BIWAKUJESZ :[");
            Toast.makeText(getApplicationContext(), "Dziś biwakujesz :]", Toast.LENGTH_SHORT).show();
        }else if (selectedDate.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY) {
            dates.add("BIWACZEK");
            dates.remove("NIE BIWAKUJESZ :[");
            Toast.makeText(getApplicationContext(), "Dziś biwakujesz :]", Toast.LENGTH_SHORT).show();
        }else {
            dates.remove("BIWAKUJESZ");
            dates.add("Niestety dziś nie ma biwaczku:[");
        }
        ArrayAdapter<String> a = new ArrayAdapter<String>(this, R.layout.list_custom_text, R.id.list_content, Arrays.asList(dates.toArray(new String[dates.size()])));
        days.setAdapter(a);
    }
